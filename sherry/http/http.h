...
private:
    ...
    template<class T>
    void setAs(MapType& m, const std::string& key, const T& val) {
        m[key] = boost::lexical_cast<std::string>(val);
    }
    template<class T>
    void delAs(MapType& m, const std::string& key) {
        m.erase(key);
    }
private:
    HttpMethod m_method;
...
private:
    ...
    HttpRequest::MapType m_headers;
    HttpRequest::MapType m_params;
    HttpRequest::MapType m_cookies;
};
