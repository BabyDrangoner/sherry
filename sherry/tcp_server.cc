...
static sherry::ConfigVar<uint64_t>::ptr g_tcp_server_read_timeout =
    sherry::Config::Lookup("tcp_server.read_timeout", (uint64_t)(60 * 1000 * 2), "tcp server read timeout");

static sherry::Logger::ptr g_logger = SYLAR_LOG_NAME("system");

TcpServer::TcpServer(sherry::IOManager* worker, sherry::IOManager* accept_worker)
    :m_worker(worker)
...
bool TcpServer::bind(const sherry::Address::ptr addr){
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs, std::vector<Address::ptr>& fails){
    ...
    for(auto& i: m_socks){
        SYLAR_LOG_INFO(g_logger) << "server bind success: " << i->getLocalAddress()->toString();
    }
    return true;
}
